...
export function DateInput({
  ...
  const [inputValue, setInputValue] = React.useState(value ? format(value, DATE_INPUT_PATTERN) : "");
  const [isCalendarOpen, setIsCalendarOpen] = React.useState(false);

  React.useEffect(() => {
    setInputValue(value ? format(value, DATE_INPUT_PATTERN) : "");
  }, [value]);
  ...
  const handleCalendarSelect = (date: Date | undefined) => {
    onChange(date);
    if (date) setInputValue(format(date, DATE_INPUT_PATTERN));
